...
local addBreakpointButton = topLayout:addChild(GUI.adaptiveButton(1, 1, 3, 1, 0xCCCCCC, 0x000000, 0x333333, 0xCCCCCC, "x"))

local syntaxHighlightingButton = topLayout:addChild(GUI.adaptiveButton(1, 1, 3, 1, 0xCCCCCC, 0x000000, 0x333333, 0xCCCCCC, "◈"))
...
continue = function(...)
	-- Preparing the screen for launch
	...
	if coroutineResumeSuccess then
		if coroutine.status(scriptCoroutine) == "dead" then
			...
			-- Test for a fag, you never know the dude also has a yield in the prog
...
local function run(...)
	...
	-- We clean shit with a fork, we clean
	if breakpointLines then
		for i = 1, #breakpointLines do
...
showTip = function(errorCode, matchCode, beep, force)
	-- Retrieve the erroneous line of the current script
	lastErrorLine = tonumber(errorCode:match(matchCode))
	if lastErrorLine then
		-- We make an adjustment for the number of breakpoints in the form of inserted debug lines
		if breakpointLines then
...
local function pasteAutoBrackets(unicodeByte)
	...
	-- If we have auto brackets turned on at all, then we check them
	if config.enableAutoBrackets then
		-- The situation when the cursor is on the closing bracket, and don't fuck it up again
		if closeBrackets[char] and currentSymbol == char then
			deleteSelectedData()
			setCursorPosition(cursorPositionSymbol + 1, cursorPositionLine)
		-- If the open bracket is pressed
		elseif openBrackets[char] then
			-- And here we take the already selected text in brackets
			if codeView.selections[1] then
